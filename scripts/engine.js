...
class Engine {
  ...
  constructor(root) {
    ...
    this.mission = new Mission(sidebar, this.player, missions[0])
  ...
  checkScreenScroll() {
    ...
    if ((this.player.x === (this.screenWidth - this.screenScrollDistance + this.horizontalOffset)) && (!(this.player.x === (WORLD_WIDTH - this.screenScrollDistance + 1)))) {
      ...
      if (this.blocks[`column_${this.player.x + this.screenScrollDistance}`].blocks.length !== 0) {
        ...
        this.blocks.biomeBuilder([this.player.x + this.screenScrollDistance]);
      }
  ...
  clockRunning() {
    setInterval(() => {
      ...
      // Victory: Filter out accomplished objectives, then check for mission objective achievements:
      this.mission.manageAchievements();
      // this.mission.objectivesRemaining.forEach(objective => {
      //   if (objective.test()) {
      //     this.mission.objectivesAchieved.push(objective);
      //     console.log("success!")
      //   };
      // })
